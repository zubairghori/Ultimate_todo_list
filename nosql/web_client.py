...
def task(id):
    requestCRUD = todoCRUD.SingleRequest(_id=id)
    ...
    if(request.method == 'DELETE'):
        ...
        requestCRUD = todoCRUD.UpdateRequest(_id=id, title=_data['title'], description=_data['description'], status = _data['status'])
        def update():
